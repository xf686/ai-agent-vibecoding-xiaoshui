...
小水的技能分两类:**新手友好技能**会根据对话内容自动上场(无需手敲命令),**进阶技能**用 `load skill <名字>` 手动加载。

### 🌱 新手友好技能(自动触发)

为编程基础薄弱的用户准备,小水会读懂你的话自动戴上对应"帽子":

| 技能 | 干啥 | 啥时候自动上场 |
|------|------|----------------|
| `error-translator` | 把吓人的报错翻成人话,给一键修复建议 | 一出现报错,或你粘贴一段看不懂的报错 |
| `env-setup` | 帮你配好运行环境、解决"装了却找不到" | "装不上""跑不起来""缺东西" |
| `requirement-coach` | 把模糊愿望问成能下手的小目标 | 需求太笼统、不知从哪开始 |
| `run-guide` | 程序做好后手把手教你怎么跑、怎么关 | 刚做好一个能跑的脚本/网页 |
| `code-explainer` | 用类比和大白话讲解代码/概念,零黑话 | "这段代码啥意思""什么是 X" |

> `error-translator` 还带**硬触发**:小水每次跑完命令会自动"闻"结果里有没有报错,一旦命中就强制切换成翻译官模式。
